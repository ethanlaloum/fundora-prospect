...
# --- Enumerer, c'est decider ------------------------------------------------------
#
# Compter est un calcul ; CHOISIR les champs a montrer en est un aussi, plus
# discret. Un ecart dont le front nommerait les clefs pourrait en perdre une au
# premier remaniement de mise en page — et celle qu'on perdrait est
# `arguments_respectes`, la CAUSE d'un ecart faux. Un ecart sans sa cause se lit
# comme une explication.
#
# Le front rend donc `Object.entries(ecart)`. La cause n'est pas affichee parce
# que quelqu'un s'en est souvenu : elle l'est parce qu'elle est dans l'objet.


def clefs_d_ecart() -> set[str]:
    """Les clefs d'un ecart, DERIVEES du coeur en le faisant tourner.

    Une liste ecrite ici serait une copie de plus — celle qui derive au premier
    champ ajoute, et le verrou cesserait de garder ce qu'il croit garder.
    """
    from fundora_prospect.agent import ecart

    # `comparer` ajoute la cause a l'ecart que `ecart` construit ; elle est
    # nommee ici parce qu'elle ne sort pas de la meme fonction.
    return set(ecart(["a"], ["a"])) | {"arguments_respectes"}


def test_le_front_ne_NOMME_aucune_clef_d_ecart() -> None:
    """Ce qui rend la separation impossible.

    Tant qu'aucune clef n'est ecrite dans `web/src`, aucune ne peut etre
    oubliee : le rendu est generique, et un champ ajoute demain a l'ecart
    apparait sans qu'on touche au JSX.
    """
    interdites = clefs_d_ecart()
    fautifs = {
        p.relative_to(RACINE).as_posix(): trouves
        for p in fichiers_web()
        if (trouves := occurrences_de_clefs(
            sans_commentaires(p.read_text(encoding="utf-8", errors="ignore")), interdites
        ))
    }
    assert not fautifs, (
        "le front nomme des clefs d'ecart — il pourrait en oublier une, et "
        "celle qu'il oublierait est la cause :\n"
        + "\n".join(f"  {f} : {sorted(t)}" for f, t in sorted(fautifs.items()))
    )


def occurrences_de_clefs(texte: str, clefs: set[str]) -> set[str]:
    return {clef for clef in clefs if re.search(rf"\b{re.escape(clef)}\b", texte)}


def test_le_verrou_attrape_une_clef_nommee() -> None:
    """**Les dents.** Le cas exact qu'on craint : quelqu'un sort la cause du
    rendu generique pour la placer ailleurs dans la mise en page."""
    jsx = 'const cause = <p>{ecart.arguments_respectes}</p>;'
    assert occurrences_de_clefs(jsx, clefs_d_ecart()) == {"arguments_respectes"}


def test_le_verrou_derive_bien_les_clefs() -> None:
    """Un ensemble vide passerait le verrou sans rien garder."""
    clefs = clefs_d_ecart()
    assert {"identiques", "meme_ordre", "arguments_respectes"} <= clefs
    assert len(clefs) >= 5
